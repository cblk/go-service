...
func getPolicyToken(callbackUrl string) (*OssBodyParams, error) {
	bodyParams := &OssBodyParams{}
	...
	condition = append(condition, cfg.Oss.PathPrefix)
	...
	callbackParam.CallbackBody = `{"mime_type":${mimeType},"size":${size},"filename":${object}}`
	...
	bodyParams.Policy = deByte
	bodyParams.OSSAccessKeyId = accessKeyId
	bodyParams.CallBack = callbackBase64
	bodyParams.Signature = signedStr
...
func GetMD5FromNewAuthString(r *http.Request, strCallbackBody string) ([]byte, error) {
	var byteMD5 []byte
